...
public class RelativeImageView extends View implements ImageObserver, MouseListener, MouseMotionListener {
    ...
    private ImageProvider fImageProvider = null;
     ...
     */
    public RelativeImageView(Element elem, ImageProvider provider) {
        super(elem);
        fImageProvider = provider;
        ...
        try {
            ...
            if (isURL()) {
                ...
                if (fImageProvider != null) {
                    try {
                        fImage = fImageProvider.resolveImageReference(src);
            ...
            if (fImage != null) {
                if (customHeight && customWidth) {
                    Toolkit.getDefaultToolkit().prepareImage(fImage, width, height, this);
                } else {
    ...
    boolean isLink() {
        ...
        return anchorAttr != null && anchorAttr.isDefined(HTML.Attribute.HREF);
    }
    ...
    private int getIntAttr(HTML.Attribute name, int iDefault) {
        AttributeSet fElementAttributes = fElement.getAttributes();
        if (fElementAttributes.isDefined(name)) {
            int i;
            String val = (String) fElementAttributes.getAttribute(name);
            if (val == null) {
                i = iDefault;
            } else {
                try {
    ...
    protected void resize(int width, int height) {
        ...
        MutableAttributeSet mattrSet = new SimpleAttributeSet();
        mattrSet.addAttribute(HTML.Attribute.WIDTH, Integer.toString(width));
        mattrSet.addAttribute(HTML.Attribute.HEIGHT, Integer.toString(height));
        ((StyledDocument) getDocument())
                .setCharacterAttributes(fElement.getStartOffset(), fElement.getEndOffset(), mattrSet, false);
    }
